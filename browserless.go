...
// Browserless represents a single execution environment.
type Browserless struct {
	ctx *engine.Context
}

// New creates a new browserless environment tied to the provided context.
func New(ctx context.Context) *Browserless {
...
func (b *Browserless) LoadHTML(htmlStr string) error {
...
func (b *Browserless) LoadURL(urlStr string) error {
...
func (b *Browserless) Evaluate(script string) error {
	b.ctx.RunScript("eval", script)
	return b.ctx.EventLoop()
}

// Close terminates the environment and releases resources.
func (b *Browserless) Close() {
	b.ctx.Close()
}

func (b *Browserless) resolveURL(base, ref string) string {
